...
static PyObject* d3d12_Device_create_texture2d_from_native(d3d12_Device* self, PyObject* args)
{
	...
	py_resource->resource->GetDevice(__uuidof(ID3D12Device1), (void**)&py_resource->device);
	py_resource->device->AddRef();

	py_resource->device->GetCopyableFootprints(&resource_desc, 0, 1, 0, &py_resource->footprint, NULL, NULL, &py_resource->size);
	py_resource->dimension = resource_desc.Dimension;
	py_resource->stride = 0;
...
static PyObject* d3d12_Device_create_compute(d3d12_Device* self, PyObject* args, PyObject* kwds)
{
	...
	py_compute->root_signature = NULL;
	py_compute->descriptor_heap = NULL;
	py_compute->pipeline = NULL;
	py_compute->queue = NULL;
	py_compute->fence = NULL;
	py_compute->fence_event = NULL;
	py_compute->fence_value = 0;
	py_compute->command_allocator = NULL;
	py_compute->command_list = NULL;
	...
	for (d3d12_Resource* resource : srv)
	{
		if (resource->dimension == D3D12_RESOURCE_DIMENSION_BUFFER)
		{
			...
			srv_desc.Buffer.NumElements = resource->size / (srv_desc.Format ? dxgi_pixels_sizes[resource->format] : 1);
...
static PyObject* d3d12_Device_create_buffer_from_native(d3d12_Device* self, PyObject* args)
{
	...
	py_resource->device->AddRef();
	py_resource->footprint = {};
	py_resource->device->GetCopyableFootprints(&resource_desc, 0, 1, 0, NULL, NULL, NULL, &py_resource->size);
	py_resource->dimension = resource_desc.Dimension;
	py_resource->stride = 0;
...
static PyObject* d3d12_Resource_copy_to(d3d12_Resource* self, PyObject* args)
{
	...
	ID3D12CommandQueue* queue;
	HRESULT hr = self->device->CreateCommandQueue(&queue_desc, __uuidof(ID3D12CommandQueue), (void**)&queue);
	if (hr != S_OK)
	{
		return d3d12_generate_exception(hr, "Unable to create Command Queue");
	}
	...
	ID3D12Fence1* fence;
	hr = self->device->CreateFence(0, D3D12_FENCE_FLAG_NONE, __uuidof(ID3D12Fence1), (void**)&fence);
	if (hr != S_OK)
	{
		queue->Release();
		return d3d12_generate_exception(hr, "Unable to create Fence");
	...
	ID3D12CommandAllocator* command_allocator;
	hr = self->device->CreateCommandAllocator(queue_desc.Type, __uuidof(ID3D12CommandAllocator), (void**)&command_allocator);
	if (hr != S_OK)
	{
		fence->Release();
	...
	ID3D12GraphicsCommandList1* command_list;
	hr = self->device->CreateCommandList(0, queue_desc.Type, command_allocator, NULL, __uuidof(ID3D12GraphicsCommandList1), (void**)&command_list);
	if (hr != S_OK)
	{
...
PyInit_d3d12(void)
{
	PyObject* m = PyModule_Create(&compushady_backends_d3d12_module);
	if (m == NULL)
		return NULL;

	d3d12_Device_Type.tp_members = d3d12_Device_members;
	d3d12_Device_Type.tp_methods = d3d12_Device_methods;
	if (PyType_Ready(&d3d12_Device_Type) < 0)
	{
		Py_DECREF(m);
		return NULL;
	}
	Py_INCREF(&d3d12_Device_Type);
	if (PyModule_AddObject(m, "Device", (PyObject*)&d3d12_Device_Type) < 0)
	{
		Py_DECREF(&d3d12_Device_Type);
		Py_DECREF(m);
		return NULL;
	}

	d3d12_Resource_Type.tp_members = d3d12_Resource_members;
	d3d12_Resource_Type.tp_methods = d3d12_Resource_methods;
	if (PyType_Ready(&d3d12_Resource_Type) < 0)
	{
		Py_DECREF(&d3d12_Device_Type);
		Py_DECREF(m);
		return NULL;
	}
	Py_INCREF(&d3d12_Resource_Type);
	if (PyModule_AddObject(m, "Resource", (PyObject*)&d3d12_Resource_Type) < 0)
	{
		Py_DECREF(&d3d12_Resource_Type);
		Py_DECREF(&d3d12_Device_Type);
		Py_DECREF(m);
		return NULL;
	}

	d3d12_Compute_Type.tp_methods = d3d12_Compute_methods;
	if (PyType_Ready(&d3d12_Compute_Type) < 0)
	{
		Py_DECREF(&d3d12_Resource_Type);
		Py_DECREF(&d3d12_Device_Type);
		Py_DECREF(m);
	...
	Py_INCREF(&d3d12_Compute_Type);
	if (PyModule_AddObject(m, "Compute", (PyObject*)&d3d12_Compute_Type) < 0)
	{
		Py_DECREF(&d3d12_Compute_Type);
		Py_DECREF(&d3d12_Resource_Type);
		Py_DECREF(&d3d12_Device_Type);
		Py_DECREF(m);
		return NULL;
