...
typedef std::pair<MTLPixelFormat, uint32_t> metal_pixel_format_t;
std::unordered_map<uint32_t, metal_pixel_format_t> metal_formats;
...
COMPUSHADY_NEW_TYPE(metal, Heap);
...
static void metal_Compute_dealloc(metal_Compute* self)
{
    ...
    self->rtv = std::vector<metal_Resource*>();
...
static PyObject* metal_Compute_draw(metal_Compute* self, PyObject* args)
{
    ...
    render_pass_descriptor.colorAttachments[0].texture = self->rtv[0]->texture;
    ...
    for (size_t i = 0; i < self->cbv.size(); i++)
    {
        ...
        if (py_resource->texture)
        {
            ...
            [render_command_encoder setFragmentTexture:py_resource->texture
                                               atIndex:texture_index++];
        }
        else
        {
            [render_command_encoder setVertexBuffer:py_resource->buffer
                                             offset:0
                                            atIndex:buffer_index];
            [render_command_encoder setFragmentBuffer:py_resource->buffer
                                               offset:0
                                              atIndex:buffer_index++];
        }
    }
    ...
    for (size_t i = 0; i < self->samplers.size(); i++)
    {
        ...
        [render_command_encoder setFragmentSamplerState:py_sampler->sampler
...
static PyObject* metal_Device_create_rasterizer(metal_Device* self, PyObject* args, PyObject* kwds)
{
    ...
    py_compute->rtv = std::vector<metal_Resource*>();
    py_compute->cbv = std::vector<metal_Resource*>();
    py_compute->srv = std::vector<metal_Resource*>();
    py_compute->uav = std::vector<metal_Resource*>();
    py_compute->samplers = std::vector<metal_Sampler*>();
    ...
    if (!compushady_check_descriptors(&metal_Resource_Type, py_rtv, py_compute->rtv)
        || !compushady_check_descriptors(&metal_Resource_Type, py_cbv, py_compute->cbv)
        || !compushady_check_descriptors(&metal_Resource_Type, py_srv, py_compute->srv)
        || !compushady_check_descriptors(&metal_Resource_Type, py_uav, py_compute->uav)
        || !compushady_check_descriptors(&metal_Sampler_Type, py_samplers, py_compute->samplers))
    {
    ...
    py_compute->py_rtv_list = PyList_New(0);
    py_compute->py_cbv_list = PyList_New(0);
    py_compute->py_srv_list = PyList_New(0);
    py_compute->py_uav_list = PyList_New(0);
    py_compute->py_samplers_list = PyList_New(0);
...
static compushady_backend_desc_t metal_backend;

PyMODINIT_FUNC PyInit_metal(void)
{

    compushady_backend_desc_init(&metal_backend, "metal", compushady_backends_metal_methods);

    metal_backend.device_type = &metal_Device_Type;
    metal_backend.device_members = metal_Device_members;
    metal_backend.device_methods = metal_Device_methods;

    metal_backend.resource_type = &metal_Resource_Type;
    metal_backend.resource_members = metal_Resource_members;
    metal_backend.resource_methods = metal_Resource_methods;

    metal_backend.swapchain_type = &metal_Swapchain_Type;
    metal_backend.swapchain_members = metal_Swapchain_members;
    metal_backend.swapchain_methods = metal_Swapchain_methods;

    metal_backend.compute_type = &metal_Compute_Type;
    metal_backend.compute_methods = metal_Compute_methods;

    metal_backend.sampler_type = &metal_Sampler_Type;

    metal_backend.heap_type = &metal_Heap_Type;
    metal_backend.heap_members = metal_Heap_members;

    PyObject* m = compushady_backend_init(&metal_backend);
