...
bool compushady_check_copy_to(const bool src_is_buffer, const bool dst_is_buffer, const uint64_t size, const uint64_t src_offset, const uint64_t dst_offset, const uint64_t src_size, const uint64_t dst_size, const uint32_t src_x, const uint32_t src_y, const uint32_t src_z, const uint32_t src_slice, const uint32_t src_slices, const uint32_t dst_slice, const uint32_t dst_slices, const uint32_t src_width, const uint32_t src_height, const uint32_t src_depth, const uint32_t dst_width, const uint32_t dst_height, const uint32_t dst_depth, uint32_t *dst_x, uint32_t *dst_y, uint32_t *dst_z, uint32_t *width, uint32_t *height, uint32_t *depth, const uint32_t src_mip, const uint32_t dst_mip, const uint32_t src_mips, const uint32_t dst_mips)
{
	// buffer to buffer
	...
	else if (src_is_buffer && !dst_is_buffer)
	{
		...
		*dst_z = 0;
		...
		if (*width == 0)
		{
			*width = dst_width >> dst_mip;
		}

		if (*height == 0)
		{
			*height = dst_height >> dst_mip;
		}
		...
		if (src_offset + size > src_size || size < dst_size || dst_slice >= dst_slices || dst_mip >= dst_mips)
		{
			PyErr_Format(PyExc_ValueError,
						 "Resource requested size to copy (%llu) is out of bounds "
						 "(src_size: %llu, src_offset: %llu, dst_size: %llu, dst_width: %u, dst_height: %u, dst_depth: %u, dst_slices: %u, dst_mips: %u)",
						 size, size, src_offset, dst_size, dst_width, dst_height, dst_depth, dst_slices, dst_mips);
			return false;
		}
	}
	// texture to buffer
	else if (!src_is_buffer && dst_is_buffer)
	{
		...
		*dst_z = 0;
		...
		if (*width == 0)
		{
			*width = src_width >> src_mip;
		}

		if (*height == 0)
		{
			*height = src_height >> src_mip;
		}
		...
		if (dst_offset + size > dst_size || size < src_size || src_slice >= src_slices || src_mip >= src_mips)
		{
			PyErr_Format(PyExc_ValueError,
						 "Resource requested size to copy (%llu) is out of bounds "
						 "(dst_size: %llu, dst_offset: %llu, src_size: %llu, src_width: %u, src_height: %u, src_depth: %u, src_slices: %u, src_mips: %u)",
						 size, dst_size, src_offset, dst_size, src_width, src_height, src_depth, src_slices, src_mips);
			return false;
		}
	}
	// texture to texture
	else
	{
		...
		if (src_x + *width > src_width || src_y + *height > src_height || src_z + *depth > src_depth || *dst_x + *width > dst_width || *dst_y + *height > dst_height || *dst_z + *depth > dst_depth || src_slice >= src_slices || dst_slice >= dst_slices || src_mips >= src_mips || dst_mip >= dst_mips)
		{
			PyErr_Format(PyExc_ValueError,
						 "Resource requested size to copy (width: %u, height: %u, depth: %u) is out of bounds "
						 "(src_width: %u, src_height: %u, src_depth: %u src_slices: %u, src_mips: %, dst_width: %u, dst_height: %u, dst_depth: %u, dst_slices: %u, dst_mips: %u)",
						 *width, *height, *depth, src_width, src_height, src_depth, src_slices, src_mips, dst_width, dst_height, dst_depth, dst_slices, dst_mips);
			return false;
		}
	}
