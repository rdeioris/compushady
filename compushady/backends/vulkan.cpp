...
static const char* vulkan_get_spirv_entry_point(const uint32_t* words, size_t len)
{
    ...
    while (offset < words_num)
    {
        ...
        if (opcode == 0x0F && (offset + size < words_num)
            && words[offset + 1] == 5) // OpEntryPoint(0x0F) + GLCompute(0x05)
        {
            if (size > 3)
            {
...
static PyObject* vulkan_instance_check()
{
    ...
    for (VkExtensionProperties& extension_prop : available_extensions)
    {
        ...
        if (vulkan_debug
            && !strcmp(extension_prop.extensionName, VK_EXT_DEBUG_UTILS_EXTENSION_NAME))
        {
...
static PyObject* vulkan_Device_create_buffer(vulkan_Device* self, PyObject* args)
{
    ...
    buffer_create_info.usage = VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT
    ...
    vulkan_Resource* py_resource
        = (vulkan_Resource*)PyObject_New(vulkan_Resource, &vulkan_Resource_Type);
    if (!py_resource)
    {
    ...
    VkResult result
        = vkCreateBuffer(py_device->device, &buffer_create_info, NULL, &py_resource->buffer);
    if (result != VK_SUCCESS)
    {
    ...
    allocate_info.memoryTypeIndex
        = vulkan_get_memory_type_index_by_flag(&self->mem_props, mem_flag);
...
static PyObject* vulkan_Device_create_texture2d(vulkan_Device* self, PyObject* args)
{
    ...
    VkResult result
        = vkAllocateMemory(py_device->device, &allocate_info, NULL, &py_resource->memory);
    if (result != VK_SUCCESS)
    {
        Py_DECREF(py_resource);
...
static PyObject* vulkan_Device_create_texture3d(vulkan_Device* self, PyObject* args)
{
    ...
    py_resource->row_pitch = width * vulkan_formats[format].second;
    py_resource->size
        = py_resource->row_pitch * height * depth; // alway assume a packed configuration
    py_resource->format = image_view_create_info.format;
...
static PyObject* vulkan_Device_create_sampler(vulkan_Device* self, PyObject* args)
{
    ...
    vulkan_Sampler* py_sampler
        = (vulkan_Sampler*)PyObject_New(vulkan_Sampler, &vulkan_Sampler_Type);
    if (!py_sampler)
    {
...
static PyObject* vulkan_Device_create_compute(vulkan_Device* self, PyObject* args, PyObject* kwds)
{
    ...
    for (vulkan_Resource* py_resource : uav)
    {
        ...
        else
        {
            ...
            {
                if (py_resource->format == VK_FORMAT_B8G8R8A8_UNORM
                    || py_resource->format == VK_FORMAT_B8G8R8A8_SRGB)
                {
    ...
    const char* entry_point
        = vulkan_get_spirv_entry_point(shader_create_info.pCode, shader_create_info.codeSize);
    if (!entry_point)
    {
        if (shader_create_info.pCode != view.buf)
            PyMem_Free((void*)shader_create_info.pCode);
    ...
    VkShaderModule shader_module;
    VkResult result
        = vkCreateShaderModule(py_device->device, &shader_create_info, nullptr, &shader_module);
    if (result != VK_SUCCESS)
    {
        if (shader_create_info.pCode != view.buf)
            PyMem_Free((void*)shader_create_info.pCode);
    ...
    vulkan_Compute* py_compute
        = (vulkan_Compute*)PyObject_New(vulkan_Compute, &vulkan_Compute_Type);
    if (!py_compute)
    {
    ...
    result
        = vkCreateDescriptorPool(py_device->device, &pool_info, NULL, &py_compute->descriptor_pool);
    if (result != VK_SUCCESS)
    {
        Py_DECREF(py_compute);
...
static PyObject* vulkan_Device_create_swapchain(vulkan_Device* self, PyObject* args)
{
    ...
    vulkan_Swapchain* py_swapchain
        = (vulkan_Swapchain*)PyObject_New(vulkan_Swapchain, &vulkan_Swapchain_Type);
    if (!py_swapchain)
    {
...
static PyMethodDef vulkan_Device_methods[] = {
    ...
    /*{"create_buffer_from_native",
       (PyCFunction)d3d12_Device_create_buffer_from_native, METH_VARARGS,
       "Creates a Buffer object from a low level pointer"},*/
    { "create_texture2d_from_native", (PyCFunction)vulkan_Device_create_texture2d_from_native,
        METH_VARARGS, "Creates a Texture2D object from a low level pointer" },
...
static PyObject* vulkan_get_discovered_devices(PyObject* self)
{
    ...
    for (auto device : devices)
    {
        ...
        py_device->is_hardware = prop.deviceType == VK_PHYSICAL_DEVICE_TYPE_INTEGRATED_GPU
            || prop.deviceType == VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU
            || prop.deviceType == VK_PHYSICAL_DEVICE_TYPE_VIRTUAL_GPU;
...
static PyObject* vulkan_Resource_upload(vulkan_Resource* self, PyObject* args)
{
    ...
    VkResult result = vkMapMemory(
        self->py_device->device, self->memory, 0, self->size, 0, (void**)&mapped_data);
    if (result != VK_SUCCESS)
    {
        PyBuffer_Release(&view);
...
static PyObject* vulkan_Swapchain_present(vulkan_Swapchain* self, PyObject* args)
{
    ...
    image_copy.extent.width
        = Py_MIN(src_resource->image_extent.width, self->image_extent.width - x);
    image_copy.extent.height
        = Py_MIN(src_resource->image_extent.height, self->image_extent.height - y);
...
static struct PyModuleDef compushady_backends_vulkan_module
    = { PyModuleDef_HEAD_INIT, "vulkan", NULL, -1, compushady_backends_vulkan_methods };

PyMODINIT_FUNC PyInit_vulkan(void)
{
    PyObject* m = compushady_backend_init(&compushady_backends_vulkan_module, &vulkan_Device_Type,
        vulkan_Device_members, vulkan_Device_methods, &vulkan_Resource_Type,
        vulkan_Resource_members, vulkan_Resource_methods, &vulkan_Swapchain_Type,
        vulkan_Swapchain_members, vulkan_Swapchain_methods, &vulkan_Compute_Type, NULL,
        vulkan_Compute_methods, &vulkan_Sampler_Type, NULL, NULL);
