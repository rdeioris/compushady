...
from compushady import (
    SHADER_BINARY_TYPE_GLSL,
    SHADER_BINARY_TYPE_MSL,
    SHADER_TARGET_TYPE_CS,
)
...
class DXCTests(unittest.TestCase):
    def test_to_glsl(self):
        ...
        self.assertTrue(b"layout(binding = 1024) uniform samplerBuffer _input;" in glsl)
        self.assertTrue(
            b"layout(binding = 2048, rgba32f) uniform writeonly image2D _output;"
            in glsl
        )
